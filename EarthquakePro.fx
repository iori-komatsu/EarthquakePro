...
// p must be ≧ 0
float3 FBM(float p) {
    p = fmod(p, FBMTextureSize * FBMTextureSize); // 0 ≦ p < FBMTextureSize^2
    float x = fmod(p, FBMTextureSize);   // 0 ≦ x < FBMTextureSize
    float y = floor(p / FBMTextureSize); // 0 ≦ y < FBMTextureSize
